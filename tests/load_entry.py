...
def main(mcsa_id):
    ...
    #a.fit(b, weighted=True, scaling_factor=None, transform=True)
    ...
    for i, pdbsite in enumerate(entry.pdbsites):
        rot, tran, rms, rms_all = pdbsite.reference_site.fit(pdbsite, weighted=True, cutoff=5, scaling_factor=None, transform=True)
